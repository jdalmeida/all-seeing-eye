...
export const alertsRouter = createTRPCRouter({
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (!parsed) {
        throw new Error("Não foi possível entender a regra. Tente outra formulação.");
      }
  ...
  events: protectedProcedure.query(async () => {
    const rows = await db.select().from(alertEvents).orderBy(desc(alertEvents.createdAt)).limit(50);
    return rows;
  }),
