...
`;
  ...
  notification.className = 'fixed top-4 right-4 bg-terminal border border-primary rounded-lg p-4 z-50 max-w-md font-mono text-sm';
...
export function useTerminalEvents(onExpand?: () => void) {
  useEffect(() => {
    ...
    return () => window.removeEventListener('expand-terminal', handleExpandTerminal);
  }, [onExpand]);
}
