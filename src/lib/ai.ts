...
export async function generateChatResponse(message: string, context?: {
  news?: Array<{
    title: string;
    source: string;
    publishedAt: string;
  ...
  try {
    ...
    if (context?.type === 'news-modal' && context.news) {
      ...
      const newsContext = context.news.map((news, index) => `
Notícia ${index + 1}:
- Título: ${news.title}
- Fonte: ${news.source}
...
Mensagem do usuário: ${message}`;
      ...
      const newsSummary = context.news.map((news, index) => `
[${index + 1}] ${news.title.substring(0, 50)}${news.title.length > 50 ? '...' : ''} - ${news.source}`).join('\n');
