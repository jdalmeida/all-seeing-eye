...
export function parseNaturalLanguageRule(text: string): ParsedAlertRule | null {
	...
	const cryptoDrop = normalized.match(
		/(btc|eth|sol|ada|dot|link|bnb)\s+(cair|queda|cair\s+mais)\s+(de\s+)?(\d+)[%％]\s+em\s+(\d+)h/,
	);
	if (cryptoDrop) {
	...
	const newsMulti = normalized.match(
		/(\w[\w\s\-\.]+)\s+aparecer\s+em\s+mais\s+de\s+(\d+)\s+fontes\s+no\s+mesmo\s+dia/,
	);
	if (newsMulti) {
